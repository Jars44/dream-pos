...
export default function VariantAttributesPage() {
  ...
  const handleSelectAll = (checked: boolean) => {
    if (checked) {
      setSelectedVariants(variantAttributesData.map((v) => v.id));
    } else {
      setSelectedVariants([]);
    }
  };

  const handleSelectVariant = (id: number, checked: boolean) => {
    if (checked) {
    ...
    if (searchQuery) {
      ...
      result = result.filter(
        (v) =>
          ...
          v.values.toLowerCase().includes(query)
      );
    }
        ...
        <div>
          ...
          <Breadcrumb className="mt-1">
            <BreadcrumbList>
              ...
              <BreadcrumbSeparator />
              <BreadcrumbItem>
                <BreadcrumbPage>Variant Attributes</BreadcrumbPage>
              </BreadcrumbItem>
            </BreadcrumbList>
          </Breadcrumb>
        </div>
          ...
          <TableHeader className="bg-zinc-200/40">
            <TableRow className="border-b hover:bg-transparent">
              <TableHead className="w-12">
                <Checkbox
                  checked={selectedVariants.length === variantAttributesData.length}
